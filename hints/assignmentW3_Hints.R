...
#' 
#' ## <span style="color:blue">Hint</span>
#' You can do that by hand. If you want to it with R, then you can check this function:
#+ eval = FALSE
?dir.create
#' 
...
#+ eval = FALSE
?setwd
#' Make sure that you check whether the change in working directory worked or not!
#' 
...
#' 
#' ## <span style="color:blue">Hint</span>
#' You can check the [R reference card](https://cran.r-project.org/doc/contrib/Baggott-refcard-v2.pdf), on
#' how to index a matrix. You have also done it for the previous assignment.
#' For chosing random numbers, then check this function
#+ eval = FALSE
?sample
#' For the rest, there are enough hints in the assignment itself.
...
#'
#' ## <span style="color:blue">Hint</span>
#' Here things start becoming a bit more interesting. For solving this you will need to 
#' use a loop. You can chose what type of loop you will like to use, but I would personally
#' go for a *for loop*. For saving the files into a txt file, check
#+ eval = FALSE
?write.table
#' If you check the help file, you will also see how you can remove quotes from the files,
#' and how you can get rib of the row.names.
#' Lastly, for changing the "X" in the "fakepart_X" part, make sure that the "X" changes
#' with every loop turn -- you can check the class notes on that.
#'
...
#+ eval = FALSE
?read.table
#' You can read the data, using a loop again. Make sure that you first create
#' an emtpy list object, with the line below:
#+ eval = FALSE
empty.list = list()
#' 
#' For changing the names of the list, check this
#' 
#+ eval = FALSE
names(empty.list)
#' and make sure that you assign the names of the files into it.
#' 
...
#' 
#' ## <span style="color:blue">Hint</span>
#' You did something similar in the previous assignment, so no hints here.
#' 
